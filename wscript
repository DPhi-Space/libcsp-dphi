...
def options(ctx):
    ...
    gr.add_option(
        ...
        action="store_true",
        help="Enable reproducible builds",
    )
    ...
    gr.add_option(
        "--disable-print-stdio",
        action="store_true",
        help="Disable vprintf for csp_print_func",
    )
    gr.add_option("--disable-stlib", action="store_true", help="Build objects only")
    gr.add_option(
        "--disable-buffer-zero-clear",
        action="store_false",
        help="Zero out the packet buffer upon allocation",
    )
    ...
    gr.add_option(
        "--enable-promisc", action="store_true", help="Enable promiscuous support"
    )
    ...
    gr.add_option(
        "--enable-rtable",
        action="store_true",
        help="Allows to setup a list of static routes",
    )
    gr.add_option(
        "--enable-python3-bindings", action="store_true", help="Enable Python3 bindings"
    )
    gr.add_option("--enable-examples", action="store_true", help="Enable examples")
    gr.add_option(
        "--enable-dedup", action="store_true", help="Enable packet deduplicator"
    )
    ...
    gr.add_option(
        ...
        type=int,
        default=5,
        help="Set maximum window size for RDP",
    )
    gr.add_option(
        "--with-max-bind-port", type=int, default=16, help="Set maximum bindable port"
    )
    gr.add_option(
        "--with-max-connections",
        type=int,
        default=8,
        help="Set maximum number of connections",
    )
    gr.add_option(
        "--with-conn-queue-length",
        type=int,
        default=15,
        help="Set max connection queue length",
    )
    gr.add_option(
        "--with-router-queue-length",
        type=int,
        default=15,
        help="Set max router queue length",
    )
    gr.add_option(
        "--with-buffer-size", type=int, default=256, help="Set size of csp buffers"
    )
    gr.add_option(
        "--with-buffer-count", type=int, default=15, help="Set number of csp buffers"
    )
    gr.add_option(
        "--with-rtable-size",
        type=int,
        default=10,
        help="Set max number of entries in route table",
    )

    # Drivers and interfaces (requires external dependencies)
    gr.add_option(
        "--enable-if-zmqhub", action="store_true", help="Enable ZMQ interface"
    )
    gr.add_option(
        "--enable-can-socketcan",
        action="store_true",
        help="Enable Linux socketcan driver",
    )
    gr.add_option(
        "--with-driver-usart",
        default=None,
        metavar="DRIVER",
        help="Build USART driver. [linux, None]",
    )
    ...
    # Fixup
    gr.add_option(
        "--fixup-v1-zmq-little-endian",
        action="store_true",
        help="Use little-endian CSP ID for ZMQ with CSPv1",
    )
...
def configure(ctx):
    ...
    git_rev = (
        os.popen("git describe --long --always 2> /dev/null || echo unknown")
        .read()
        .strip()
    )
    ...
    ctx.env.append_unique(
        "INCLUDES_CSP", ["include", "src"] + ctx.options.includes.split(",")
    )
    ...
    # Add if UDP
    if ctx.check(header_name="sys/socket.h", mandatory=False) and ctx.check(
    ...
    if ctx.options.enable_rdp:
        ctx.env.append_unique("FILES_CSP", ["src/csp_rdp.c", "src/csp_rdp_queue.c"])

    # Add YAML
    if ctx.options.enable_yaml:
        ctx.check_cfg(
            package="yaml-0.1", args="--cflags --libs", define_name="CSP_HAVE_LIBYAML"
        )
    ...
    if ctx.options.enable_if_zmqhub:
        ctx.check_cfg(
            package="libzmq", args="--cflags --libs", define_name="CSP_HAVE_LIBZMQ"
        )
...
def build(ctx):
    ...
    if ctx.is_install:
        ...
        ctx.install_files(
            "${PREFIX}/include/csp", "include/csp/autoconfig.h", cwd=ctx.bldnode
        )
    ...
    if ctx.env.ENABLE_EXAMPLES:
        ctx.objects(
            source="examples/csp_posix_helper.c", target="csp_posix_helper", use="csp_h"
        )

        ctx.program(
            source="examples/csp_server_client.c",
        ...
        ctx.program(
            source=["examples/csp_bridge_can2udp.c"],
