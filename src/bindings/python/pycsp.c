...
static int CSP_POINTER_HAS_BEEN_FREED =
    0;  // used to indicate pointer has been freed, because a NULL pointer can't be set.
...
static void* get_capsule_pointer(PyObject* capsule, const char* expected_type, bool allow_null) {
    ...
    if (strcmp(capsule_name, expected_type) != 0) {
        PyErr_Format(PyExc_TypeError, "capsule contains unexpected type, expected=%s, got=%s", expected_type,
                     capsule_name);  // TypeError is thrown
        return NULL;
    }
    ...
    if ((ptr == NULL) && !allow_null) {
        PyErr_Format(PyExc_TypeError, "capsule: type=%s - already freed",
...
static PyObject* pycsp_init(PyObject* self, PyObject* args) {
    if (!PyArg_ParseTuple(args, "sss|bIb", &csp_conf.hostname, &csp_conf.model, &csp_conf.revision, &csp_conf.version,
                          &csp_conf.conn_dfl_so, &csp_conf.dedup)) {
        return NULL;
    }
...
static PyObject* pycsp_transaction(PyObject* self, PyObject* args) {
    ...
    if (!PyArg_ParseTuple(args, "bHbIw*w*|i", &prio, &dest, &port, &timeout, &outbuf, &inbuf,
                          &allow_any_incoming_length)) {
        return NULL;  // TypeError is thrown
    }
...
static int csp_route_start_task(void) {
    ...
    pthread_attr_setdetachstate(&attributes,
                                PTHREAD_CREATE_DETACHED);  // no need to join with thread to free its resources
...
static PyObject* pycsp_rdp_set_opt(PyObject* self, PyObject* args) {
    ...
    if (!PyArg_ParseTuple(args, "IIIIII", &window_size, &conn_timeout_ms, &packet_timeout_ms, &delayed_acks,
...
#if (CSP_USE_RDP)
    csp_rdp_get_opt(&window_size, &conn_timeout_ms, &packet_timeout_ms, &delayed_acks, &ack_timeout, &ack_delay_count);
#endif
    return Py_BuildValue("IIIIII", window_size, conn_timeout_ms, packet_timeout_ms, delayed_acks, ack_timeout,
                         ack_delay_count);
}
...
static PyObject* pycsp_cmp_ident(PyObject* self, PyObject* args) {
    ...
    return Py_BuildValue("isssss", res, msg.ident.hostname, msg.ident.model, msg.ident.revision, msg.ident.date,
                         msg.ident.time);
}
...
static PyObject* pycsp_cmp_clock_get(PyObject* self, PyObject* args) {
    ...
    return Py_BuildValue("II", be32toh(msg.clock.tv_sec), be32toh(msg.clock.tv_nsec));
}
...
static struct PyModuleDef moduledef = {PyModuleDef_HEAD_INIT, "libcsp_py3", NULL, -1, methods, NULL, NULL, NULL, NULL};
